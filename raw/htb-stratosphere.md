[HTB: Stratosphere](/2018/09/01/htb-stratosphere.html)
...
[![Stratosphere](/icons/box-stratosphere.webp)](https://hackthebox.com/machines/stratosphere)

[Stratosphere](https://hackthebox.com/machines/stratosphere)
...
Rated Difficulty ![Rated difficulty for Stratosphere](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/stratosphere-diff.webp)

Radar Graph ![Radar chart for Stratosphere](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/stratosphere-radar.webp)

User

03:09:30 Root

04:05:33 Creator ## nmap
...
Site is for credit protection: ![Stratosphere Web80 Root image](https://0xdfimages.gitlab.io/img/stratosphere-web80-root.png)
...
`/Monitoring` returns a 302 which takes us to `http://10.10.10.64/Monitoring/example/Welcome.action`: ![Stratosphere Web80 Monitoring Root image](https://0xdfimages.gitlab.io/img/stratosphere-web80-monitoring-root.png)
...
The sign on button goes to `http://10.10.10.64/Monitoring/example/Login_input.action`, which returns a login page: ![Stratosphere Web80 Monitoring Login image](https://0xdfimages.gitlab.io/img/stratosphere-web80-monitoring-login.png)

There’s also a hello world app at `http://10.10.10.64/Monitoring/example/HelloWorld.action` that doesn’t seem to do much: ![Stratosphere Web80 Monitoring Helloworld image](https://0xdfimages.gitlab.io/img/stratosphere-web80-monitoring-helloworld.png)

## Apache Struts - RCE

At this point we have a ton of hints to look at Struts:

- All the paths in the Monitoring example application end with.action. That is a very meaningful word in the Apache Struts framework. Here’s an overview (taken from [the NetBeans docs](https://netbeans.org/kb/docs/web/quickstart-webapps-struts.html)): ![Stratosphere Struts image](https://0xdfimages.gitlab.io/img/stratosphere-struts.png)
...
There were a bunch of high profile vulnerabilities in Struts recently, several of which provide RCE: https://www.cvedetails.com/vulnerability-list/vendor\_id-45/product\_id-6117/Apache-Struts.html
...
At this point, we’d like to get a better shell, and we’d like to get a user with more privilege. The low priv user, tomcat8, can’t access a lot. There is a home dir for richard. In `/etc/tomcat8/tomcat-users/xml`, there’s a username and password:
...
Outbound traffic is heavily filtered - I’m not able to `wget` or `nc` to attacker box. `ifconfig`, `netstat`, `curl` not on host. Perhaps this is a container, or just stripped down?

There are more creds in local dir, `/var/lib/tomcat8/db_connect`:
...
`/proc/net/fib_trie` shows ip of 10.10.10.64 and localhost. `/proc/net/tcp` shows 6 listening ports:
...
That’s interesting because the original nmap showed 22, 80, and 8080. That result above shows the following ports listening:
