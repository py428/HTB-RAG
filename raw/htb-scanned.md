[HTB: Scanned](/2022/09/10/htb-scanned.html)

![Scanned](https://0xdfimages.gitlab.io/img/scanned-cover.webp)
...
[![Scanned](/icons/box-scanned.webp)](https://hackthebox.com/machines/scanned)

[Scanned](https://hackthebox.com/machines/scanned)
...
Rated Difficulty ![Rated difficulty for Scanned](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/scanned-diff.webp)

Radar Graph ![Radar chart for Scanned](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/scanned-radar.webp)

User

02:35:47 Root

18:36:12 Creator ## Recon
...
![image-20220909124135548](https://0xdfimages.gitlab.io/img/image-20220909124135548.webp)
...
![image-20220111161739114](https://0xdfimages.gitlab.io/img/image-20220111161739114.webp)
...
![image-20220111162249809](https://0xdfimages.gitlab.io/img/image-20220111162249809.webp)
...
![image-20220111191458336](https://0xdfimages.gitlab.io/img/image-20220111191458336.webp)
...
![image-20220907133006925](https://0xdfimages.gitlab.io/img/image-20220907133006925.webp)
...
![img](https://0xdfimages.gitlab.io/img/Chroot-1.webp)
...
Why does it matter that the OS is Debian? When the kernel is going to trace another process or access another processes `/proc/`, it looks at `/proc/sys/kernel/yama/ptrace_scope` (detailed [in the Yama documentation](https://www.kernel.org/doc/Documentation/security/Yama.txt)). In most distros, this is set to `1` by default, which means that you can only trace or access descendant processes. So PID 1 can trace 2 or 3, but neither 2 nor 3 can access any of the others. But Debian has decided (reasons detailed [in this Debian bug report](https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=712740)) to default to `0`, which means that a process can attach (and/or access `/proc`) to any other process running under the same UID, as long as it is dumpable.
...
![image-20220907141700007](https://0xdfimages.gitlab.io/img/image-20220907141700007.webp)
...
![image-20220907142040142](https://0xdfimages.gitlab.io/img/image-20220907142040142.webp)
...
![image-20220907142608161](https://0xdfimages.gitlab.io/img/image-20220907142608161.webp)
...
![](https://www.youtube.com/watch?v=nGm36Hd-p4s)
...
![image-20220907153245255](https://0xdfimages.gitlab.io/img/image-20220907153245255.webp)
...
The hash is of the format described [in the passlib docs](https://passlib.readthedocs.io/en/stable/lib/passlib.hash.django_std.html#id3), `{ident}${salt}${hash}`. The hash is calculated by combining the salt and then the password.
...
![image-20220112165850279](https://0xdfimages.gitlab.io/img/image-20220112165850279.webp)
...
I could go down trying to fix these by implementing the version info and the `pam_putenv` function (described [in the pam\_putenv man page](https://man7.org/linux/man-pages/man3/pam_putenv.3.html)).
...
The definition for this function is [in the misc\_conv man page](https://man7.org/linux/man-pages/man3/misc_conv.3.html):
