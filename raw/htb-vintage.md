[HTB: Vintage](/2025/04/26/htb-vintage.html)

![Vintage](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/vintage-cover.webp)
...
[![Vintage](/icons/box-vintage.webp)](https://hackthebox.com/machines/vintage)

[Vintage](https://hackthebox.com/machines/vintage)
...
Rated Difficulty ![Rated difficulty for Vintage](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/vintage-diff.webp)

Radar Graph ![Radar chart for Vintage](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/vintage-radar.webp)

User

01:26:17 Root

03:16:52 Creator Scenario
...
Based on [these ports](about:/cheatsheets/os#windows-domain-controller) it looks like a Windows Domain Controller. The domain name `vintage.htb` is present, as well as the hostname DC01. I’ll add these to my `hosts` file (in the proper order):
...
I didn’t expect `bloodhound` to work without setting it up for Kerberos auth (like I showed in [Ghost](about:/2025/04/05/htb-ghost.html#enumeration)), but it does:
...
I’ll [start the Bloodhound CE docker container](about:/2025/03/15/htb-certified.html#setup) and load the data.
...
![image-20250418155330138](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20250418155330138.webp)
...
![image-20250424084032768](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20250424084032768.webp)
...
As a member of the Domain Computers group, FS01 can read the GMSA password for the GMSA01$ service account. Typically `netexec` is the best way to do this (as shown on [HTB Mist](about:/2024/10/26/htb-mist.html#get-password-hash)), but it fails here:
...
![image-20250418165256468](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20250418165256468.webp)

That group has three members:

![image-20250418165331804](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20250418165331804.webp)

It also has `GenericAll` over three service accounts:

![image-20250418165455073](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20250418165455073.webp)
...
![image-20250418165927050](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20250418165927050.webp)
...
![image-20250420144107377](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20250420144107377.webp)
...
Now I’ll use the [Impacket](https://github.com/SecureAuthCorp/impacket) `dpapi.py` script (installed with `uv tool install impacket`, see [this video](https://www.youtube.com/watch?v=G36QXtBXKBQ) for Python tool installations) to first decrypt the master key:
...
![image-20250421085218854](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20250421085218854.webp)

Switching to Pathfinding from C.Neri\_adm to DC01 shows the full privileges:

![image-20250421090816390](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20250421090816390.webp)

### RBCD Delegation

#### Strategy

The `AllowedToAct` attribute is given when the group is configured for resource based constrained delegation (RBCD). I’ve covered this before in [Freelancer](about:/2024/10/05/htb-freelancer.html#exploit-via-rbcd) and [Rebound](about:/2024/03/30/htb-rebound.html#resource-based-constrained-delegation).
...
![image-20250421100638146](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20250421100638146.webp)
