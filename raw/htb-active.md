...
Rated Difficulty ![Rated difficulty for Active](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/active-diff.webp)

Radar Graph ![Radar chart for Active](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/active-radar.webp)

User

00:05:37 Root

01:06:00 Creators   ## Recon
...
In a Kerberoasting attack, rather than sending the encrypted ticket from the DC to the service, you will use off-line brute force to crack the password associated with the service. Typically, Kerberoasting requires a valid domain account, since the attacker has to authenticate to the KDC to request a TGS for the target SPN.

There is a flag on user accounts in Active Directory, “Do not require Kerberos preauthentication”, that is used for accounts that, for legacy or compatibility reasons, cannot perform Kerberos pre-authentication. Accounts with this configuration allow anyone to request an AS-REP for that account without authentication. An AS-REP is the response from the KDC to a client’s initial authentication request. It contains a TGT and a session key (and other metadata) that is encrypted with the user’s password hash. The user would need to know the password to decrypt this key and use the TGT. [AS-REP Roasting](https://harmj0y.medium.com/roasting-as-reps-e6179a65216b) is an attack where an attacker requests an AS-REP for a user configured this way and brute forces the decryption offline. If the account password is weak, the attacker will know when the encrypted section of the AS-REP decrypts properly.
...
I’ll look up the hash type [on the hashcat wiki](https://hashcat.net/wiki/doku.php?id=example_hashes), and then crack it with hashcat:
