[HTB: Infiltrator](/2025/06/14/htb-infiltrator.html)

![Infiltrator](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/infiltrator-cover.webp)
...
[![Infiltrator](/icons/box-infiltrator.webp)](https://hackthebox.com/machines/infiltrator)

[Infiltrator](https://hackthebox.com/machines/infiltrator)
...
Rated Difficulty ![Rated difficulty for Infiltrator](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/infiltrator-diff.webp)

Radar Graph ![Radar chart for Infiltrator](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/infiltrator-radar.webp)

User

01:44:28 Root

03:05:51 Creator ## Recon
...
There’s a ton there. I’ll note the standard [DC ports](about:/cheatsheets/os#windows-domain-controller) (DNS on 53, Kerberos on 88, LDAP on 389 and other, SMB on 445, RPC on 135, and NetBios on 139). The hostname `DC01` and domain name `infiltrator.htb` are both leaked. There’s also a couple remote access ports, WinRM (5985) and remote desktop (RDP, 3389). There’s also an HTTP server on 80, and the title of that page is “Infiltrator.htb”. I’ll do a quick fuzz with `ffuf` to look for subdomains that respond differently, but not find any. I’ll add update my `hosts` with with `10.10.11.31    DC01.infiltrator.htb infiltrator.htb DC01`.
...
The site is for a digital marketing firm:
 ...
 ![image-20240903155946880](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240903155946880.webp)![expand](/icons/expand.png)
...
![image-20240903160609509](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240903160609509.webp)
...
![image-20240905060700725](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240905060700725.webp)

That is `GenericAll` over the Marketing Digital OU:

![image-20240905060840575](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240905060840575.webp)

Working down the chain of outbound control, I’ll find this graph:

![image-20240905061058454](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240905061058454.webp)
...
![image-20240906092735187](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906092735187.webp)
...
With `FullControl`, a lot of people would go to changing the password of e.rodriguez. I would much rather add a shadow credential to the account using [Certipy](https://github.com/ly4k/Certipy?tab=readme-ov-file#shadow-credentials) like I showed in [Absolute](about:/2023/05/27/htb-absolute.html#shadow-credential). This attack is not only stealthier, but also isn’t suceptable to HTB cleanup scripts.
...
![image-20240906163921244](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906163921244.webp)
...
![image-20240906164221860](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906164221860.webp)
...
![image-20240906164333318](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906164333318.webp)

#### Unknown Site - TCP 14126

This page returns a directory listing:

![image-20240906164614033](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906164614033.webp)

The `output` directory returns a 404 page:

![image-20240906164635878](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906164635878.webp)

### Get UserExplorer.exe

#### Output Messenger as K.Turner

The user enumeration [above](about:/2025/06/14/htb-infiltrator.html#user-enumeration) showed the password “MessengerApp@Pass!” for K.Turner, which works here to log in:

![image-20240906165159425](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906165159425.webp)

The General\_Chat room has a single message from the admin:

![image-20240906165852671](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906165852671.webp)

The Dev\_Chat room has a bunch of chat about LDAP and programming to use it in their app. It does mention “our Output Wall”:

![image-20240906170018586](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906170018586.webp)

There’s also a message from admin asking for the final copy of the app:

![image-20240906170444500](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906170444500.webp)

#### Output Wall

Researching [Output Wall](https://support.outputmessenger.com/output-wall-integration/), it’s a plugin to the chat application designed for interoffice communication. I’ll download the Output messenger application from [the Output Messenger site](https://www.outputmessenger.com/lan-messenger-downloads/) and install it on my Linux VM.
...
![image-20240906180627086](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906180627086.webp)

On entering K.Turner’s creds, it shows a similar interface with additional options:

![image-20240906173821757](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906173821757.webp)

The chat is nicer looking:

![image-20240906173846037](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906173846037.webp)
...
![image-20240906174029857](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906174029857.webp)

The wall has messages on it:

![image-20240906174757554](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906174757554.webp)

The post from K.Turner has M.Harris’ password on it:

![image-20240906174827981](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906174827981.webp)
...
![image-20240906180953052](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906180953052.webp)
...
![image-20240906183422295](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906183422295.webp)
...
![image-20240906210438639](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906210438639.webp)

Interestingly, the output is a base64 encoded string. It decodes to junk:

![image-20240906210842473](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906210842473.webp)

But if I decrypt again, it returns a string:

![image-20240906211139296](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240906211139296.webp)
...
![image-20240907065650909](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907065650909.webp)

They are in a group, Management and Security:

![image-20240907065917074](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907065917074.webp)

They have a chat history with O.Martinez:

![image-20240907065733720](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907065733720.webp)
...
![image-20240907065948437](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907065948437.webp)

And A.Walker requested access to a group chat:

![image-20240907070012710](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907070012710.webp)
...
![image-20240907070122797](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907070122797.webp)

### Output Messenger API

#### Authentication

[This page](https://support.outputmessenger.com/authentication-api/) from the Output Messenger documentation shows how to use the API on 14125, which matches the API identified [above](about:/2025/06/14/htb-infiltrator.html#api---tcp-14125). Hitting it without auth returns an error:
...
![image-20240907070440537](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907070440537.webp)
...
There’s an API to get logs from a room, but it requires the “Chat Room key”, which I have it from the database file [above](about:/2025/06/14/htb-infiltrator.html#db-enumeration).
...
![image-20240907072917494](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907072917494.webp)
...
![image-20240907074859370](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907074859370.webp)
...
![image-20240907075924291](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907075924291.webp)

These reminders are probably what is popping up every day as they are set to open a website:

![image-20240907080145432](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907080145432.webp)
...
![image-20240907080303819](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907080303819.webp)

However, when I switch to Windows:

![image-20240907092606748](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907092606748.webp)
...
![image-20240907092431935](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907092431935.webp)
...
![image-20240907093747663](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907093747663.webp)
...
![image-20240907094147990](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907094147990.webp)
...
![image-20240907101843036](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907101843036.webp)

They can RDP into DC01:

![image-20240907101812678](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907101812678.webp)
...
![image-20240907095947942](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907095947942.webp)

Statistics –> Protocol Hierarchy shows it’s mostly HTTP and TLS:

![image-20240907100059619](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907100059619.webp)
...
![image-20240907100217695](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907100217695.webp)
...
![image-20240907100438268](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907100438268.webp)

At the bottom of the page is this footer:

![image-20240907100529090](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907100529090.webp)

It’s a match on [this repo](https://github.com/bennyscripts/flask-file-hosting).

The next request is a POST to `/login`:

![image-20240907100642524](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907100642524.webp)

The password is “securepassword”, which matches the default from the repo:

![image-20240907101538873](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907101538873.webp)
...
![image-20240907101402048](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907101402048.webp)

There’s also HTML indicating there’s one file uploaded:

![image-20240907100856338](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907100856338.webp)

It’s named `BitLocker-backup.7z`. A few streams later, there’s a GET to `/view/raw/BitLocker-backup.7z`:

![image-20240907101051931](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907101051931.webp)

Under File –> Export Objects –> HTTP, the dialog shows packet 107 having the 209 kB file:

![image-20240907101220052](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907101220052.webp)
...
![image-20240907101458994](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907101458994.webp)
...
![image-20240907105039176](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907105039176.webp)
...
![image-20240907102430498](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907102430498.webp)
...
![image-20240907102811508](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907102811508.webp)

#### Access E:

Opening Explorer, there’s an `E:` that’s locked:

![image-20240907102827676](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907102827676.webp)

Double-clicking it pops a Bitlocker prompt:

![image-20240907102900120](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907102900120.webp)

I’ll click “More options”:

![image-20240907105149400](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907105149400.webp)

“Enter recovery key” offers a field for the key:

![image-20240907105215257](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907105215257.webp)

Entering the key from the backup works:

![image-20240907105242017](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907105242017.webp)
...
![image-20240907105942646](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907105942646.webp)
...
![image-20240907154550774](https://pub-4caceed5c57c4466b559b0834d2806c9.r2.dev/img/image-20240907154550774.webp)

### Dump Hash

A group Managed Service Account (gMSA) is an account that has it’s password managed automatically by the domain. It allows these accounts to set very strong passwords, and gives permissions to other accounts to access them as needed. I’ve shown this a couple times, most recently on [Rebound](about:/2024/03/30/htb-rebound.html#auth-as-delegator).
...
I recently showed the ESC4 exploit on [EscapeTwo](about:/2025/05/24/htb-escapetwo.html#) (which came out way after Infiltrator). ESC4 says that the infiltrator\_svc$ account has excessive permissions over a template, `Infiltrator_Template`. This account can modify it to make it vulnerable to other ESC attacks.
